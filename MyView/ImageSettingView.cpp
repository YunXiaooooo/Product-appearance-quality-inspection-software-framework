...
namespace View {
		...
		switch (mSettingProgressStatus)
		{
			case View::ImageSettingView::PROGRESS_LOAD_MODEL: {
				...
				if (mSettingModelName.size() == 0) {	// 判断加载模型名称规范
					QMessageBox::warning(NULL, QStringLiteral("warning"), QStringLiteral("Failed to load model!"), QMessageBox::Yes);
				}
				else {
					...
					QMessageBox::information(NULL, QStringLiteral("Tips"), "Load Model " + mSettingModelName + "!", QMessageBox::Yes);
				}
			...
			case View::ImageSettingView::PROGRESS_NEW_MODEL: {
				...
				if (mSettingModelName.size() == 0) {	// 判断创建新模型名称规范
					QMessageBox::warning(NULL, QStringLiteral("warning"), QStringLiteral("Failed to create model, please check the name!"), QMessageBox::Yes);
					mSettingProgressStatus = View::ImageSettingView::PROGRESS_INITIAL_NONE;

				}
				else {
					curModelName = ui.lineEditNewModel->text().toStdString();
					//新建模型的时候，无法在加载下拉栏里选择当前模型，重启后生效
					//// 将当前名称添加到已有的模型box当中
					...
					//}
					QMessageBox::information(NULL, QStringLiteral("Tips"), "Create New Model " + mSettingModelName + "!", QMessageBox::Yes);
					// 更换按钮文本
					ui.buttonNewModel->setText(QStringLiteral("finish"));
					// 使能检测参数设置
			...
			case View::ImageSettingView::PROGRESS_NEW_MODEL_FINISH: {
				// 更换按钮文本
				ui.buttonNewModel->setText(QStringLiteral("New"));
				// 保存数据入库
				saveRelatedParam();
				// 失能参数设置
	...
	void ImageSettingView::saveRelatedParam() {
		QMessageBox::StandardButton rs = QMessageBox::information(NULL, QStringLiteral("Note"), QStringLiteral("Whether to save the current model parameters?"), QMessageBox::Yes);
		...
		if (mSettingProgressStatus == View::ImageSettingView::PROGRESS_INITIAL_NONE) {	// 初始状态，负责显示连续图像视频
			if (tabIdx < mExternalCameraNums) {
				if (mCamerasDisplayTimer != nullptr) {
					...
					QMessageBox::StandardButton rs = QMessageBox::information(NULL, QStringLiteral("Note"), QStringLiteral("Switching cameras..."), QMessageBox::Yes);
					if (rs == QMessageBox::Yes) {
	...
	void ImageSettingView::onCamerasDisplayTimerTimeOut() {
		...
		if (tabIdx < mExternalCameraNums) {
			...
			{
				QMessageBox::warning(NULL, QStringLiteral("警告"), QStringLiteral("Camera failed to get image!"), QMessageBox::Yes);
			}
		}
		else {	// 切换到空白标签页
			ui.labelDisplay->clear();
...
namespace View {
	...
	: groupBoxName(groupBoxName), lineEditNames(lineEditNames){
		...
		if (radioButtonYes == NULL)	// 启动检测QRadioButton
			radioButtonYes = new QRadioButton(QStringLiteral("Start"));
		if (radioButtonNo == NULL) {	// 关闭检测QRadioButton
			radioButtonNo = new QRadioButton(QStringLiteral("Close"));
