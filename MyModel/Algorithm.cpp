...
namespace Model {
	...
	std::vector<bool> AlgorithmInterface::taskRun(std::vector<cv::Mat> &camerasMat,
		...
		*/
		// (TODO)使用线程池
		std::queue < std::future<bool>> checkResultFeature;//存储多线程结果
		std::vector<cv::Mat> thisCameraMat(taskNum);
		for (int i = 0; i < taskNum; ++i)
		{	// 测试从0开始
			thisCameraMat[i] = camerasMat[mBlockToCameraIndex[i]]
			(cv::Range(mBlockParam[i][1], mBlockParam[i][3]), cv::Range(mBlockParam[i][0], mBlockParam[i][2]));		// 取出block图像
			// 获取单个任务的每个检测需求
			int singleCheckStatus = 0;
			for (int singleCheckIdx = 0; singleCheckIdx < mDetectTypeSettingNames.size(); ++singleCheckIdx)
			{
				...
				if (singleCheckStatus == 1) {
					switch (singleCheckIdx)
					{
						// 当前只有六个测试检测类型
					case 0: {
						checkResultFeature.push(pool->enqueue(threadCheck1TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					case 1: {
						checkResultFeature.push(pool->enqueue(threadCheck2TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					case 2: {
						checkResultFeature.push(pool->enqueue(threadCheck3TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					case 3: {
						checkResultFeature.push(pool->enqueue(threadCheck4TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					case 4: {
						checkResultFeature.push(pool->enqueue(threadCheck5TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					case 5: {
						checkResultFeature.push(pool->enqueue(threadCheck6TestRegularRunningProcess, &(*this), thisCameraMat[i]));
						break;
					}
					default:
						break;
		...
		for (int i = 0; i < taskNum; ++i)
		{
			...
			for (int singleCheckIdx = 0; singleCheckIdx < mDetectTypeSettingNames.size(); ++singleCheckIdx)
			{
				...
				bool singleCheckResult = true;
				if (singleCheckStatus == 1) {
					std::future<bool> singleCheckResultFuture = std::move(checkResultFeature.front());
					singleCheckResult = singleCheckResultFuture.get();
					checkResultFeature.pop();
					if (!singleCheckResult) {
						errorCheckCateIndex[i].push_back(singleCheckIdx);
					}
				}
